shoppingApp.service('teaInventoryService', ['$resource', function($resource) {
  ...
  this.assembleCategories = function() {
    var array = [];
    for (var i = 0; i < this.getTeas.length; i++) {
      for (var j = 0; j < this.getTeas[i].categories.length; j++) {
        if (array.indexOf(this.getTeas[i].categories[j]) == -1){
          array.push(this.getTeas[i].categories[j])
      }
    }
  }
  return array
